...
class Client {
  ...
  async init(gotModule) {
    if (gotModule) {
      got = gotModule;
    } else {
      // eslint still doesn't support dynamic import, `parsing error` cannot be
      // suppressed by `eslint-disable`
  ...
  async runTest(projectId, testId, initiator) {
    const that = this;
    const opt = this.getDefaultOptions();
    opt.searchParams.append('initiator', (_.isEmpty(initiator) ? INITIATOR : initiator));
  ...
  async getTestRunStatusPolling(runId, time = 5000) {
    ...
    const noResultDetailedStatus = ['SYSTEM_ERROR', 'ABORTED'];
    const hasReportUIStatus = ['FAILED', 'PASSED', 'STOPPED', 'HALTED'];
    ...
    do {
      try {
        ...
        const getReport = async (reportType) => {
          ...
          const resultPath = path.join(artifacts_folder, `./results_run_${runId}.${utils.getFileExtensionByReportType(reportType)}`);
          const report = await that.createTestRunReport(runId, reportType).catch((err) => {
            that.logger.error(`creating run report (${reportType}) failed: ${err.message}`);
          });
          if (_.isSafeInteger(_.get(report, 'reportId'))) {
            return that.getTestRunReportPolling(resultPath, report.reportId, REPORT_POLLING_INTERVAL).catch((err) => {
              that.logger.error(`checking run report (${reportType}) failed: ${err.message}`);
            });
          }
   ...
   * @return {Promise<void>}
   */
  async generateJUnitXmlReport(runData) {
